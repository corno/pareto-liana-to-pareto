...
const location = sh.v.component_imported(
    "location",
    "Range",
)
        ...
        p_.from.dictionary(
            p_.literal.dictionary({
                "location": p_.literal.optionals_dictionary({
                    "": p_.literal.set(
                        sh.schema_reference.sr.external(
                            "astn-core",
                            "deserialization",
                            "location",
                        ))
    ...
    return p_.from.state($).decide(
        ($) => {
            switch ($[0]) {
                ...
                case 'dictionary': return p_.option($, ($) => sh.v.group(
                    p_.literal.dictionary({
                        "l location": location,
                        "l dictionary": sh.v.dictionary(
                            sh.v.group(
                                p_.literal.dictionary({
                                    "l location": location,
                                    "l entry": Value(
                ...
                case 'reference': return p_.option($, ($) => p_.from.state($.type).decide(
                    ($) => {
                        switch ($[0]) {
                            case 'derived': return p_.option($, ($) => sh.v.nothing())
                            case 'selected': return p_.option($, ($) => {
                                return sh.v.group(
                                    p_.literal.dictionary({
                                        "l location": location,
                                        "l reference": sh.v.text(),
                                    })
                                )
                            })
                            default: return p_.exhaustive($[0])
                ...
                case 'state': return p_.option($, ($) => sh.v.group(
                    p_.literal.dictionary({
                        "l location": location,
                        "l state": sh.v.state(p_.from.dictionary($.options).map(
                            ($, id) => Value(
                                $.value,
                            )))
                    })
